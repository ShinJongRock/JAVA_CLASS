...
public class Controller implements Initializable { // 모델과 뷰를 컨트롤하는 클래스
	...
	@FXML
	private Button first_go_button; // 처음으로 돌아가기
		...
		try {
			...
			// 정상적으로 진행될 때
				...
				for (int i = 0; i < yutnoriSystem.playingPlayer.size(); i++) { // 플레이어별 말 현황 출력
					totalPieceNumber = yutnoriSystem.playingPlayer.get(i).onBoardPieceNumber
							+ yutnoriSystem.playingPlayer.get(i).notOnBoardPieceNumber;
					playerPieceStatus.get(i)
							.setText("현재: " + yutnoriSystem.playingPlayer.get(i).onBoardPieceNumber + " / 총: "
	...
	private void newPieceClicked(MouseEvent event) {
		...
		disableMainSquares(); // 보드 위의 모든 칸을 선택 불가능하게 변경하는 함수
	...
	private void listViewSetOnMouseClicked(MouseEvent event) {
		...
		for (int i = 1; i <= (yutnoriSystem.board.squares)[currentSquare].pieces.size(); i++) { // 원래 있던 square의
																								// circle들을 안보이게
		...
		// 모든 플레이어의 턴이 끝나면 게임 종료
			...
			for (int i = 0; i < yutnoriSystem.playingPlayer.size(); i++) { // 플레이어별 말 현황 출력
				totalPieceNumber = yutnoriSystem.playingPlayer.get(i).onBoardPieceNumber
						+ yutnoriSystem.playingPlayer.get(i).notOnBoardPieceNumber;
				playerPieceStatus.get(i).setText("현재: " + yutnoriSystem.playingPlayer.get(i).onBoardPieceNumber
						+ " / 총: " + totalPieceNumber + " / 골인: " + yutnoriSystem.playingPlayer.get(i).goalInNumber);
			}
		...
		// 이동 했는데 골인 안했을 경우 도착 square의 circle들을 보이게
			...
			for (int i = 1; i <= (yutnoriSystem.board.squares)[nextSquare].pieces.size(); i++) { // 목표 칸에 실제 존재하는 말들만
																									// 보이게 하기
				temp = subSquares[nextSquare][i];
				temp.setVisible(true);
				temp.setFill(color);
		...
		if (yutList.getItems().size() == 0) {
			if (yutnoriSystem.playingPlayer.get(turn).eatAndRollAgain == false) {
				...
				boolean initialized = yutnoriSystem.board.initializePiece(turn); // 다음 차례가 보드 위에 말이 하나도 없을 경우 자동으로 놓기.
																					// 이미 존재하는 말을 먹을 수 있다.
				if (initialized == true) {
	...
	@FXML
	void first_button(ActionEvent event) throws IOException {
	...
	public void initialize(URL url, ResourceBundle resourceBundle) { // 뷰 요소들과 자료구조 초기화
		...
		static_Label = player1; // 이름 표시
		static_Label = player2;
		static_Label = player3; // 이름 표시
		static_Label = player4;

		yutnoriSystem = new YutnoriSystem(); // 윷놀이 시스템(최종모델)을 나타내는 오브젝트
...
//						System.out.println(
//								"접속 성공" + socket.getRemoteSocketAddress() + ": " + Thread.currentThread().getName());
//						String message = new String(buffer, 0, length, "UTF-8");
//						for (Controller Controller : Main.clients) {
...
//					try {
//						System.out.println(
//								"수신오류" + socket.getRemoteSocketAddress() + ": " + Thread.currentThread().getName());
//
//					} catch (Exception e2) {
//						e2.printStackTrace();
//					}
...
//					
//					try {
//						System.out.println("송신오류"
//											+socket.getRemoteSocketAddress() 
//											+ ": " + Thread.currentThread().getName());
//						Main.clients.remove(Controller.this);
//						socket.close();
//					}catch(Exception e2) {
//						e2.printStackTrace();
//					}

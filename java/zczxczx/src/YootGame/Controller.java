...
public class Controller implements Initializable {      // 모델과 뷰를 컨트롤하는 클래스
    ...
    File file2 = new File("src/YootGame/dice2.png"); // 도
    File file3 = new File("src/YootGame/dice3.png");	// 개
    File file4 = new File("src/YootGame/dice4.png");	// 걸
    File file6 = new File("src/YootGame/dice6.png");	// 윷
    File file5 = new File("src/YootGame/dice5.png");	// 모
    File file1 = new File("src/YootGame/dice1.png");	// 백도
    @FXML public void doChat(ActionEvent event) {
    ...
    @FXML private void startGameButtonClicked(ActionEvent event) {
        ...
        try {
            ...
            // 정상적으로 진행될 때
                ...
                System.out.println(yutnoriSystem.playingPlayer.get(0));
            }
        }
        // 숫자가 아닌 입력값일때
            ...
            public void run() {
               try {
                  for (int i = 0; i < 20; i++) { // 주사위 또로로로롤 번
                     File files = new File("src/YootGame/dice" + (random.nextInt(6) + 1) + ".png"); // 이미지 파일 불러오기
    ...
    @FXML private void listViewSetOnMouseClicked(MouseEvent event){
        ...
        // 이동하고 골인했을 경우 남아있는 yutList를 초기화
        	...
        	yutnoriSystem.playingPlayer.get(turn).deletePieceOnBoard();
//        	int totalPieceNumber;
////            for(int i = 0; i < yutnoriSystem.playingPlayer.size()-1; i++){ // 플레이어별 말 현황 출력
////                totalPieceNumber = yutnoriSystem.playingPlayer.get(i).onBoardPieceNumber + yutnoriSystem.playingPlayer.get(i).notOnBoardPieceNumber;
////                playerPieceStatus.get(i).setText("현재: " + (yutnoriSystem.playingPlayer.get(i).onBoardPieceNumber+2) + " / 총: "
////                        + (totalPieceNumber+1) + " / 골인: " + (yutnoriSystem.playingPlayer.get(i).goalInNumber));
...
//            yutList.getItems().clear();
        ...
        if(yutList.getItems().size() ==  0){
            if(yutnoriSystem.playingPlayer.get(turn).eatAndRollAgain == false){
                ...
                else if(yutnoriSystem.playingPlayer.get(turn).notOnBoardPieceNumber > 0 && yutnoriSystem.playingPlayer.get(turn).onBoardPieceNumber ==0){
                	newPiece.setDisable(true);
                }
                else {
                    newPiece.setDisable(false);
    ...
    @FXML private void boardOnMouseClicked(MouseEvent event){
        ...
        if(yutnoriSystem.board.squares[currentSquare].pieces.size() == 0 || yutnoriSystem.board.squares[currentSquare].pieces.get(0).player != turn){
            alert = new Alert(Alert.AlertType.INFORMATION);

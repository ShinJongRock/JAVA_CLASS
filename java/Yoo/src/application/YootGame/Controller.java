...
public class Controller implements Initializable {      // 모델과 뷰를 컨트롤하는 클래스
    ...
    File file2 = new File("src/YutGame/dice2.png");   // 도
    File file3 = new File("src/YutGame/dice3.png");   // 개
    File file4 = new File("src/YutGame/dice4.png");   // 걸
    File file6 = new File("src/YutGame/dice6.png");   // 윷
    File file5 = new File("src/YutGame/dice5.png");   // 모
    File file1 = new File("src/YutGame/dice1.png");   // 백도
    ...
    // 게임 시작 버튼을 클릭했을때
    @FXML public void startGameButtonClicked(ActionEvent event) {
            ...
            public void run() {
               try {
                  for (int i = 0; i < 20; i++) { // 주사위 또로로로롤 번
                     File files = new File("src/YutGame/dice" + (random.nextInt(6) + 1) + ".png"); // 이미지 파일 불러오기
    ...
    @FXML private void listViewSetOnMouseClicked(MouseEvent event){
        ...
        int totalPieceNumber;
        // 플레이어별 말 현황 출력
        for(int i = 0; i < yutnoriSystem.playingPlayer.size(); i++) {
            totalPieceNumber = yutnoriSystem.playingPlayer.get(i).onBoardPieceNumber + yutnoriSystem.playingPlayer.get(i).notOnBoardPieceNumber;
            playerPieceStatus.get(i).setText("현재: " + yutnoriSystem.playingPlayer.get(i).onBoardPieceNumber + " / 총: "
                    + totalPieceNumber + " / 골인: " + yutnoriSystem.playingPlayer.get(i).goalInNumber);
        }
